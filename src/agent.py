...
# 修改学期结束日期为实际日期（根据当前校历调整）
# 假设本学期于2026年7月15日结束
SEMESTER_END_DATE = datetime(2026, 7, 15)  # ← 修改这里
...
def get_weeks_from_date(today=None):
    """根据日期计算当前周数"""
    if today is None:
        today = datetime.now()
...
def agent_chat(user_input, context=None):
    ...
    if intent == "get_week":
        ...
        if week:
            # 获取当前周数和剩余周数
            current_week = get_weeks_from_date()
...
if __name__ == "__main__":
    ...
    # 测试1：查询周数
    print("\n【测试1】查询周数")
    result = agent_chat("现在第几周？")
    print(f"用户：现在第几周？")
    print(f"助手：{result}")
    print(f"结果：{'✅ 通过' if '第' in result and '周' in result else '❌ 失败'}")
    
    # 测试2：计算绩点
    print("\n【测试2】计算绩点")
    result = agent_chat("绩点计算85,90,78")
    print(f"用户：绩点计算85,90,78")
    print(f"助手：{result}")
    print(f"结果：{'✅ 通过' if '3.00' in result else '❌ 失败'}")
    
    # 测试3：上下文理解
    print("\n【测试3】上下文理解")
    mock_context = [
        {"role": "assistant", "content": "📅 现在是第 43 周"}
    ]
    result = agent_chat("那距离期末还有几周？", context=mock_context)
    print(f"用户：那距离期末还有几周？")
    print(f"助手：{result}")
    # 验证：回答中应包含周数和剩余周数
    if "43" in result and "周" in result:
        print("✅ 通过")
    else:
        print("❌ 失败")
